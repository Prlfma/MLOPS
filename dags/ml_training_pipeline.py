    ...
    if experiment:
        ...
        if runs:
            ...
            client.transition_model_version_stage(
                name="MusicPopularityModel", version=mv.version, stage="Staging"
            )
...
with DAG(
    ...
    check_data = BashOperator(
        task_id="check_dvc_status", bash_command="dvc status", cwd=PROJECT_ROOT
    )
    prepare_data = BashOperator(
        task_id="prepare_data", bash_command="dvc repro prepare", cwd=PROJECT_ROOT
    )
    train_model = BashOperator(
        task_id="train_model", bash_command="dvc repro train", cwd=PROJECT_ROOT
    )
    evaluate_model = BranchPythonOperator(
        task_id="evaluate_model", python_callable=check_model_quality
    )
    register_model = PythonOperator(
        task_id="register_model", python_callable=register_best_model
    )
